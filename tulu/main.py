...
prefix_ids = tokenizer.encode(
    " on a fine sunny"
)  # generate text starting with ' on a fine sunny'
suffix_ids = tokenizer.encode(
    " in the park.<|endoftext|>"
)  # generate text ending with ' in the park.<|endoftext|>'
prompt_ids = tokenizer.encode(
    "<|endoftext|> on a fine sunny"
)  # prompt the base model with the '<|endoftext|>' token and the prefix
...
dfa_graphs = [
    ctrlg.DFA_concatenate(dfa_graphs)
...
generated_ids = ctrlg.rank_generated_ids(
    base_model, generated_ids, prompt_ids, suffix_ids
)
